...
public:
    ...
    void print();
    void findAndAdd(int dataToFind, int valueToAdd);
    void deleteFirst();
    void deleteLast();
...
template <class T> 
void LinkedList<T>::findAndAdd(int dataToFind, int valueToAdd) {
    ...
    while (ptr != NULL) {
        ...
        if (ptr->data == dataToFind) {
            addAtPosition(i+1, valueToAdd); 
            break;
        }
        ptr = ptr -> next;
        i++;
